...
public class TeacherController {
	...
	//POST - CREATE 
	    ...
	    public ResponseEntity<?> createTeacher(@RequestBody Teacher teacher, UriComponentsBuilder ucBuilder) {
	        ...
	        headers.setLocation(ucBuilder.path("/v1/teachers/{id}").buildAndExpand(teacher.getIdTeacher()).toUri());
	...
	public ResponseEntity<byte[]> uploadTeacherImage(@RequestParam("id_teacher") Long idTeacher, 
		...
		// Controlar que si al crear Teacher no envian la imagen entonces se envia una cadena por defecto.
		if(teacher.getAvatar()==null){
			teacher.setAvatar("default");
		}
	...
	/* -- Forma de ejecutar esto de abajo en POSTMAN -- Asignar una socialMedia a un teacher.
	    {
	      "idTeacher":1,
	      "teacherSocialMedias":[
		   {
			  "nickname":"@tavorestrepo",
			  "socialMedia":{
			      "idSocialMedia":2
			  }
		   }
